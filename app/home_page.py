...
with st.sidebar:
    st.session_state['genres_selected'] = st.multiselect(
        ":blue[What genres would you like!]",
        st.session_state['genres']
    )
        
    types_df: pd.DataFrame = st.session_state['types']
    st.session_state['types_selected'] = st.multiselect(
        ":blue[What formats do you want to account for?]",
        types_df['type'].to_list()
    )
    ...
    st.session_state["members_ranges"] = st.slider(
        ...
        value = (st.session_state['original_anime_df']["members"].min(), st.session_state['original_anime_df']["members"].max()),
        format = "%.0f"  
    )
    ...
    st.session_state["id_ranges"] = st.slider(
        ...
        value = (st.session_state['original_anime_df']["anime_id"].min(), st.session_state['original_anime_df']["anime_id"].max()),
        format = "%.0f"  
    )
...
if st.session_state['genres_selected']:
    ...
    anime_df_to_process = anime_df_to_process[anime_df_to_process['genre'].str.contains(genres_string)]

if st.session_state['types_selected']:
    anime_df_to_process = anime_df_to_process[anime_df_to_process['type'].isin(st.session_state['types_selected'])]

if st.session_state['members_ranges']:
    anime_df_to_process = anime_df_to_process[anime_df_to_process['members'].between(st.session_state['members_ranges'][0], st.session_state['members_ranges'][1], inclusive="both")]

if st.session_state['id_ranges']:
    anime_df_to_process = anime_df_to_process[anime_df_to_process['anime_id'].between(st.session_state['id_ranges'][0], st.session_state['id_ranges'][1], inclusive="both")]
...
for col in anime_df_to_process.columns:
    if col == "rating":
        ...
        column_configs[col] = st.column_config.Column(
            ...
            help=f"The {col} of the Anime."
        )

with st.expander("Anime Dataframe", icon=":material/dataset:"):
    st.dataframe(anime_df_to_process, 
                 hide_index=True,
